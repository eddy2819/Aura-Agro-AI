...
class _GanadoScreenState extends State<GanadoScreen> {
  ...
  void _showAddAnimalDialog(BuildContext context, {Animal? animalToEdit}) {
    ...
    final tagController = TextEditingController(
      text: animalToEdit?.tag.replaceAll('#', ''),
    );
    final weightController = TextEditingController(
      text: animalToEdit?.weightKg.toString(),
    );
    final prodController = TextEditingController(
      text: animalToEdit?.productionLiters?.replaceAll(' L/dia', ''),
    );
    final descController = TextEditingController(
      text: animalToEdit?.description,
    );

    // Controladores adicionales
    final colorController = TextEditingController(
      text: animalToEdit?.color ?? 'Blanco con Negro',
    );
    final birthDateController = TextEditingController(
      text: animalToEdit?.birthDate ?? '2024-01-01',
    );
    final fatherController = TextEditingController(
      text: animalToEdit?.geneticFather,
    );
    final motherController = TextEditingController(
      text: animalToEdit?.geneticMother,
    );
    final geneticLineController = TextEditingController(
      text: animalToEdit?.geneticLine,
    );
    final originController = TextEditingController(text: animalToEdit?.origin);
    final vetResponsibleController = TextEditingController(
      text: animalToEdit?.vetResponsible,
    );
    final prevDiseasesController = TextEditingController(
      text: animalToEdit?.prevDiseases,
    );
    final allergiesController = TextEditingController(
      text: animalToEdit?.allergies,
    );
    final currentMedicationController = TextEditingController(
      text: animalToEdit?.currentMedication,
    );
    final dailyConsumptionController = TextEditingController(
      text: animalToEdit?.dailyConsumption?.toString() ?? '12.0',
    );
    final fincaController = TextEditingController(
      text: animalToEdit?.finca ?? 'Finca El Paraíso',
    );
    final loteController = TextEditingController(
      text: animalToEdit?.lote ?? 'Lote A',
    );
    final potreroController = TextEditingController(
      text: animalToEdit?.potrero ?? 'Potrero 1',
    );
    final gpsCoordsController = TextEditingController(
      text: animalToEdit?.gpsCoords,
    );
    final purchaseValueController = TextEditingController(
      text: animalToEdit?.purchaseValue?.toString(),
    );
    final purchaseDateController = TextEditingController(
      text: animalToEdit?.purchaseDate,
    );
    final providerController = TextEditingController(
      text: animalToEdit?.provider,
    );
    ...
    String selectedProductionObjective =
        animalToEdit?.productionObjective ?? 'Producción láctea';
          ...
          builder: (modalContext, setModalState) {
            ...
            List<Widget> tabPages = [
                ...
                children: [
                  ...
                  if (selectedImagePath != null) ...[
                      ...
                      child: iaAnalyzing
                          ...
                          : TextButton.icon(
                              onPressed: () async {
                                ...
                                final analysis = await SupabaseService.instance
                                    .analyzeAnimalRegistrationPhoto(
                                      selectedImagePath!,
                                    );
                                if (!modalContext.mounted) return;

                                if (analysis == null) {
                                  setModalState(() => iaAnalyzing = false);
                                  ScaffoldMessenger.of(
                                    modalContext,
                                  ).showSnackBar(
                                    const SnackBar(
                                      content: Text(
                                        'No se pudo analizar la foto. Comprueba tu conexión e inténtalo nuevamente.',
                                      ),
                                    ),
                                  );
                                  return;
                                ...
                                String? textValue(String key, [String? alias]) {
                                  final value =
                                      analysis[key] ??
                                      (alias == null ? null : analysis[alias]);
                                ...
                                setModalState(() {
                                  ...
                                  final breed = textValue('breed', 'raza');
                                  final sex = textValue('sex', 'sexo');
                                  final purpose = textValue(
                                    'purpose',
                                    'proposito',
                                  );
                                  final stage = textValue('stage', 'etapa');
                                  ...
                                  final condition = numberValue(
                                    'body_condition_estimated',
                                    'body_condition',
                                  );
                                  final production = numberValue(
                                    'production_liters_estimated',
                                    'production_liters',
                                  );

                                  final validCategory = allowedValue(
                                    category,
                                    categories,
                                  );
                                  final validBreed = allowedValue(
                                    breed,
                                    breeds,
                                  );
                                  final validSex = allowedValue(sex, const {
                                    'Macho',
                                    'Hembra',
                                  });
                                  final validPurpose = allowedValue(
                                    purpose,
                                    purposes,
                                  );
                                  final validStage = allowedValue(
                                    stage,
                                    stages,
                                  );
                                  ...
                                  if (validBreed != null) {
                                    selectedBreed = validBreed;
                                    appliedFields++;
                                  }
                                  if (validSex != null) {
                                    selectedSex = validSex;
                                    appliedFields++;
                                  }
                                  ...
                                  if (condition != null) {
                                    bodyCondition = condition.clamp(1.0, 5.0);
                                    appliedFields++;
                                  }
                                  if (production != null && production >= 0) {
                                    prodController.text = production
                                        .toStringAsFixed(1);
                                    appliedFields++;
                                  }
                                  iaAnalyzing = false;
                                  aiSuggestionsApplied = appliedFields > 0;
                                });
                                ...
                                ScaffoldMessenger.of(modalContext).showSnackBar(
                                  SnackBar(
                                    content: Text(
                                      appliedFields > 0
                                          ? '$appliedFields sugerencias aplicadas. Revisa y cambia cualquier dato antes de guardar.'
                                          : 'La función aura-ai no devolvió un análisis visual. Debe implementarse analyze_animal_registration.',
                                      style: AppTextStyles.bodyBold.copyWith(
                                        color: Colors.white,
                                      ),
                                    ),
                                    backgroundColor: appliedFields > 0
                    ...
                    if (aiSuggestionsApplied)
                      Padding(
                        ...
                        child: Text(
                          'Valores estimados por IA · Puedes editar todos los campos.',
                  ...
                  TextFormField(
                    ...
                    validator: (value) {
                      if (value == null || value.trim().isEmpty)
                        return 'Ingrese el número del arete';
                  ...
                  DropdownButtonFormField<String>(
                    ...
                    items: const [
                      DropdownMenuItem(
                        value: 'Vaca Lechera',
                        child: Text('Vaca Lechera'),
                      ),
                      DropdownMenuItem(
                        value: 'Toro Reproductor',
                        child: Text('Toro Reproductor'),
                      ),
                      DropdownMenuItem(
                        value: 'Ternero',
                        child: Text('Ternero'),
                      ...
                      DropdownMenuItem(
                        value: 'Vaca de cria',
                        child: Text('Vaca de cria'),
                      ),
                      DropdownMenuItem(value: 'Torete', child: Text('Torete')),
                      DropdownMenuItem(
                        value: 'Toro Engorde',
                        child: Text('Toro Engorde'),
                      ),
                      DropdownMenuItem(
                        value: 'Vaca de carne',
                        child: Text('Vaca de carne'),
                      ),
                      DropdownMenuItem(
                        value: 'Vaquilla',
                        child: Text('Vaquilla'),
              ...
              Column(
                ...
                children: [
                  ...
                  DropdownButtonFormField<String>(
                    ...
                    onChanged: (val) =>
                        setModalState(() => selectedPurpose = val!),
                  ),
                  ...
                  DropdownButtonFormField<String>(
                    ...
                    items: const [
                      ...
                      DropdownMenuItem(
                        value: 'Gestación',
                        child: Text('Gestación'),
                      ),
                      DropdownMenuItem(
                        value: 'Lactancia',
                        child: Text('Lactancia'),
                      ),
                    ],
                    onChanged: (val) =>
                  ...
                  if (isLechera) ...[
                    TextFormField(
                      ...
                      decoration: InputDecoration(
                        labelText:
                            'Producción Diaria de Leche (Litros - Opcional)',
                        labelStyle: AppTextStyles.caption,
                        prefixIcon: const Icon(
                          LucideIcons.milk,
                          size: 20,
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Slider(
                          ...
                          onChanged: (val) =>
                              setModalState(() => bodyCondition = val),
                        ),
                        Text(
                          '1: Caquéctico, 3: Óptimo, 5: Obeso',
                          style: AppTextStyles.caption,
                        ),
                      ],
                    ),
              ...
              Column(
                ...
                children: [
                  Text(
                    '3. Información Genética y Ubicación',
                    style: AppTextStyles.bodyBold.copyWith(
                      color: AppColors.primaryGreen,
                    ),
                  ),
                  ...
                  TextFormField(
                    ...
                    decoration: InputDecoration(
                      labelText:
                          'Coordenadas GPS (Opcional - ej: -4.008, -79.201)',
                      labelStyle: AppTextStyles.caption,
                      prefixIcon: const Icon(
                        LucideIcons.mapPin,
                        size: 20,
              ...
              Column(
                ...
                children: [
                  Text(
                    '4. Información Sanitaria e Inicial',
                    style: AppTextStyles.bodyBold.copyWith(
                      color: AppColors.primaryGreen,
                    ),
                  ),
                  ...
                  DropdownButtonFormField<String>(
                    ...
                    items: const [
                      ...
                      DropdownMenuItem(
                        value: 'Cuarentena',
                        child: Text('Cuarentena'),
                      ),
                    ],
                    onChanged: (val) =>
                        setModalState(() => selectedHealthStatus = val!),
                  ),
                  const SizedBox(height: 12),
                  ...
                  DropdownButtonFormField<String>(
                    ...
                    items: const [
                      ...
                      DropdownMenuItem(
                        value: 'Concentrado',
                        child: Text('Concentrado'),
                      ),
                      DropdownMenuItem(value: 'Mixto', child: Text('Mixto')),
                    ],
                    onChanged: (val) =>
                        setModalState(() => selectedDietType = val!),
                  ),
                  const SizedBox(height: 12),
                  ...
                  DropdownButtonFormField<String>(
                    ...
                    items: const [
                      ...
                      DropdownMenuItem(
                        value: 'Ganancia de peso',
                        child: Text('Ganancia de peso'),
                      ),
                      DropdownMenuItem(
                        value: 'Producción láctea',
                        child: Text('Producción láctea'),
                      ),
                    ],
                    onChanged: (val) =>
                        setModalState(() => selectedProductionObjective = val!),
                  ),
                  const SizedBox(height: 12),
                      ...
                      Row(
                        ...
                        children: [
                          Checkbox(
                            ...
                            activeColor: AppColors.primaryGreen,
                            onChanged: (val) =>
                                setModalState(() => balancedFeed = val!),
                          ),
              ...
              Column(
                ...
                children: [
                  ...
                  SwitchListTile(
                    ...
                    subtitle: Text(
                      'Habilitar para visualización en Marketplace',
                      style: AppTextStyles.caption,
                    ),
                    value: availableForSale,
                    activeColor: AppColors.primaryGreen,
                    onChanged: (val) =>
                        setModalState(() => availableForSale = val),
                  ),
                  const SizedBox(height: 20),

                  Text(
                    'Fotografías y Documentos Adicionales',
                    style: AppTextStyles.bodyBold.copyWith(
                      color: AppColors.primaryGreen,
                    ),
                  ),
                    ...
                    children: [
                      ElevatedButton.icon(
                        onPressed: () {
                          ...
                          ScaffoldMessenger.of(modalContext).showSnackBar(
                            const SnackBar(
                              content: Text('Foto Frontal Adjuntada'),
                        ...
                        style: ElevatedButton.styleFrom(
                          backgroundColor: imageFrontPath != null
                              ? AppColors.primaryGreen
                              : AppColors.background,
                          foregroundColor: imageFrontPath != null
                              ? Colors.white
                      ...
                      ElevatedButton.icon(
                        onPressed: () {
                          ...
                          ScaffoldMessenger.of(modalContext).showSnackBar(
                            const SnackBar(
                              content: Text('Foto Lateral Adjuntada'),
                              duration: Duration(seconds: 1),
                            ),
                          );
                        },
                        ...
                        style: ElevatedButton.styleFrom(
                          backgroundColor: imageSidePath != null
                              ? AppColors.primaryGreen
                              : AppColors.background,
                          foregroundColor: imageSidePath != null
                              ? Colors.white
                              : AppColors.textPrimary,
                        ),
                      ),
                    ...
                    children: [
                      ElevatedButton.icon(
                        onPressed: () {
                          ...
                          ScaffoldMessenger.of(modalContext).showSnackBar(
                            const SnackBar(
                              content: Text('Certificado Adjuntado'),
                              duration: Duration(seconds: 1),
                            ),
                          );
                        },
                        ...
                        style: ElevatedButton.styleFrom(
                          backgroundColor: certSanitaryPath != null
                              ? AppColors.primaryGreen
                              : AppColors.background,
                          foregroundColor: certSanitaryPath != null
                              ? Colors.white
                              : AppColors.textPrimary,
                        ),
                      ),
                      ElevatedButton.icon(
                        onPressed: () {
                          ...
                          ScaffoldMessenger.of(modalContext).showSnackBar(
                            const SnackBar(
                              content: Text('Ficha Compra Adjuntada'),
                              duration: Duration(seconds: 1),
                            ),
                          );
                        },
                        ...
                        style: ElevatedButton.styleFrom(
                          backgroundColor: docPurchasePath != null
                              ? AppColors.primaryGreen
                              : AppColors.background,
                          foregroundColor: docPurchasePath != null
                              ? Colors.white
                              : AppColors.textPrimary,
                        ),
                      ),
                                ...
                                child: ElevatedButton(
                                  onPressed: () async {
                                    if (formKey.currentState!.validate()) {
                                      ...
                                      final cleanTag = tagController.text
                                          .trim();

                                      // Verificar arete único en edición
                                      final exists = provider.animals.any(
                                        (a) => a.id == cleanTag,
                                      );
                                      if (exists &&
                                          (animalToEdit == null ||
                                              animalToEdit.id != cleanTag)) {
                                        ScaffoldMessenger.of(
                                          modalContext,
                                        ).showSnackBar(
                                          SnackBar(
                                            content: Text(
                                              'Ya existe un animal con el arete #$cleanTag',
                                              style: AppTextStyles.bodyBold
                                                  .copyWith(
                                                    color: Colors.white,
                                                  ),
                                      ...
                                      String? production;
                                      if (isLechera &&
                                          prodController.text.isNotEmpty) {
                                        production =
                                            '${double.parse(prodController.text).toStringAsFixed(1)} L/dia';
                                      }
                                      ...
                                      final savedAnimal = Animal(
                                        ...
                                        vaccineStatus:
                                            animalToEdit?.vaccineStatus,
                                        hasAlert:
                                            animalToEdit?.hasAlert ?? false,
                                        ...
                                        geneticLine:
                                            geneticLineController.text.isEmpty
                                            ? null
                                            : geneticLineController.text.trim(),
                                        origin: originController.text.isEmpty
                                            ? null
                                            : originController.text.trim(),
                                        healthStatus: selectedHealthStatus,
                                        vetResponsible:
                                            vetResponsibleController
                                                .text
                                                .isEmpty
                                            ? null
                                            : vetResponsibleController.text
                                                  .trim(),
                                        prevDiseases:
                                            prevDiseasesController.text.isEmpty
                                            ? null
                                            : prevDiseasesController.text
                                                  .trim(),
                                        allergies:
                                        ...
                                        dailyConsumption: double.tryParse(
                                          dailyConsumptionController.text,
                                        ),
                                        productionObjective:
                                            selectedProductionObjective,
                                        finca: fincaController.text,
                                        lote: loteController.text,
                                        potrero: potreroController.text,
                                        gpsCoords:
                                            gpsCoordsController.text.isEmpty
                                            ? null
                                            : gpsCoordsController.text.trim(),
                                        purchaseValue: double.tryParse(
                                      ...
                                      if (modalContext.mounted) {
                                        ...
                                        ScaffoldMessenger.of(
                                          ...
                                          SnackBar(
                                            content: Text(
                                              animalToEdit != null
                                                  ? 'Se han guardado los cambios de ${savedAnimal.name}${savedAnimal.availableForSale ? ' y actualizado su anuncio' : ''}'
                                                  : 'Se ha registrado a ${savedAnimal.name} con éxito${savedAnimal.availableForSale ? ' y publicado en el Marketplace' : ''}',
                                              style: AppTextStyles.bodyBold
                                                  .copyWith(
                                                    color: Colors.white,
                                                  ),
    ...
    return Consumer<DataProvider>(
      builder: (context, provider, child) {
        ...
        if (_activeFilter == 'Vacas') {
          filteredAnimals = provider.animals
              .where(
                (a) =>
                    a.category.contains('Vaca') ||
                    a.category.contains('Vaquilla'),
              )
              .toList();
        } else if (_activeFilter == 'Toros') {
          filteredAnimals = provider.animals
              .where((a) => a.category.contains('Toro'))
              .toList();
        } else if (_activeFilter == 'Terneros') {
          filteredAnimals = provider.animals
              .where((a) => a.category.contains('Ternero'))
              .toList();
            ...
            children: [
              ...
              if (filteredAnimals.isEmpty)
                ...
                ...filteredAnimals.map((a) {
                  final activePlans = provider.nutritionPlans
                      .where(
                        (p) =>
                            p.status == 'Activo' &&
                            (p.targetGroup == a.category ||
                                p.targetGroup == a.id ||
                                p.targetGroup == a.name),
                      )
                      .toList();
                  double? compliance;
                  ...
                  return Padding(
                    ...
                    child: AnimalCard(
                      ...
                      onWeightTap: () => _showWeightRegisterSheet(
                        context,
                        a,
                        provider,
                        isFromDetail: false,
  ...
  void _showAnimalDetailSheet(
    BuildContext context,
    Animal animal,
    DataProvider provider,
  ) {
    final List<WeightRecord> animalWeights = provider.weightRecords
        .where((r) => r.animalId == animal.id)
        .toList();
    animalWeights.sort((a, b) => b.date.compareTo(a.date));

    final List<ProductionRecord> animalProds = provider.productionRecords
        .where((r) => r.animalId == animal.id)
        .toList();
    animalProds.sort((a, b) => b.date.compareTo(a.date));

    final bool isNotified = provider.alerts.any(
      (AlertModel al) =>
          al.animalTag == animal.tag &&
          al.title.contains('Vacunación Solicitada'),
    );

    final vaccineStatusLower = animal.vaccineStatus?.toLowerCase() ?? '';
    final isVaccineVencida = vaccineStatusLower == 'vencida';
    final isVaccinePorVencer =
        vaccineStatusLower == 'por vencer' ||
        vaccineStatusLower.contains('próxima') ||
        vaccineStatusLower.contains('vence pronto') ||
        animal.vaccineStatus == null;
      ...
      builder: (sheetContext) {
        return StatefulBuilder(
          builder: (modalContext, setModalState) {
            ...
            if (animal.category.contains('Toro'))
              categoryIcon = LucideIcons.flame;
            if (animal.category.contains('Ternero'))
              categoryIcon = LucideIcons.heart;

            // IA Recordatorios dinámicos según etapa
            String iaReminder =
                "Programar pesaje de control y revisión de ración en 20 días.";
            if (animal.stage == 'Ternero') {
              iaReminder =
                  "IA Sugiere: Control de desparasitación y refuerzo de complejo B en 10 días.";
            } else if (animal.stage == 'Lactancia' ||
                animal.category == 'Vaca Lechera') {
              iaReminder =
                  "IA Sugiere: Control de mastitis clínico preventivo mañana. Incrementar balanceado en 1 kg por producción de leche.";
            } else if (animal.stage == 'Gestación') {
              iaReminder =
                  "IA Sugiere: Suplementar con sales minerales pre-parto. Apartar a potrero de maternidad en 3 semanas.";
            }
                ...
                child: Column(
                  ...
                  children: [
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(
                                '${animal.breed} • ${animal.sex} • ${_calculateAge(animal.birthDate)}',
                                style: AppTextStyles.caption,
                              ),
                            ],
                          ),
                      ...
                      child: Row(
                        children: [
                          _buildDetailTabButton(
                            ...
                            'Dashboard',
                            activeDetailTab,
                            (val) => setModalState(() => activeDetailTab = val),
                          ),
                          _buildDetailTabButton(
                            1,
                            'Sanidad y Dieta',
                    ...
                    if (activeDetailTab == 0) ...[
                          ...
                          children: [
                            ...
                            Column(
                              ...
                              children: [
                                ...
                                Text(
                                  'Potrero: ${animal.potrero} ${animal.gpsCoords != null ? "(${animal.gpsCoords})" : ""}',
                                  style: AppTextStyles.caption,
                                ),
                              ],
                            ),
                            ...
                            Row(
                              ...
                              children: [
                                ...
                                Container(
                                  ...
                                  decoration: BoxDecoration(
                                    color: animal.healthStatus == 'Saludable'
                          ...
                          children: [
                            ...
                            Row(
                              ...
                              children: [
                                ...
                                Text(
                                  '${animal.dailyConsumption?.toStringAsFixed(1) ?? "12"} kg/día',
                                  style: AppTextStyles.bodyBold,
                                ),
                              ],
                            ),
                              ...
                              SizedBox(
                                ...
                                child: isNotified
                                    ...
                                    : ElevatedButton.icon(
                                        onPressed: () async {
                                          await provider
                                              .sendVaccineNotification(animal);
                                          setModalState(() {});
                                        },
                                        icon: const Icon(
                                          LucideIcons.send,
                          ...
                          children: [
                            Row(
                              ...
                              children: [
                                ...
                                Text(
                                  animal.geneticFather ?? 'Sin registro',
                                  style: AppTextStyles.bodyBold,
                                ),
                              ],
                            ),
                            ...
                            Row(
                              ...
                              children: [
                                ...
                                Text(
                                  animal.geneticMother ?? 'Sin registro',
                                  style: AppTextStyles.bodyBold,
                                ),
                              ],
                            ),
                            ...
                            Row(
                              ...
                              children: [
                                ...
                                Text(
                                  animal.geneticLine ?? 'Sin registro',
                                  style: AppTextStyles.bodyBold,
                                ),
                              ],
                            ),
                      ...
                      Text(
                        'Información de Compra y Ventas',
                          ...
                          children: [
                            Row(
                              ...
                              children: [
                                ...
                                Text(
                                  animal.purchaseValue != null
                                      ? '\$${animal.purchaseValue!.toStringAsFixed(2)} USD'
                                      : 'Nacido en finca',
                                  style: AppTextStyles.bodyBold,
                                ),
                              ],
                          ...
                          child: ElevatedButton.icon(
                            onPressed: () async {
                              await PdfExportService.instance
                                  .exportTraceabilityCertificate(animal);
                            },
                            icon: const Icon(
                              LucideIcons.qrCode,
                              size: 16,
                          ...
                          child: ElevatedButton.icon(
                            onPressed: () async {
                              final vaccines = provider.vaccines
                                  .where((v) => v['animal_id'] == animal.id)
                                  .toList();
                              final treatments = provider.medicalTreatments
                                  .where((t) => t['animal_id'] == animal.id)
                                  .toList();
                              await PdfExportService.instance
  ...
  void _showWeightRegisterSheet(
    BuildContext context,
    Animal animal,
    DataProvider provider, {
    required bool isFromDetail,
  }) {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
            ...
            Future.delayed(const Duration(milliseconds: 2200), () {
              if (sheetContext.mounted) {
                ...
                setModalState(() {
                  ...
                  descController.text =
                      "Ternero Holstein macho registrado por voz";
                  setCategory("Ternero");
                  setBreed("Holstein");
                  setSex("Macho");
                  setStage("Ternero");
  ...
  void _showExitDetailsDialog(
    ...
    showDialog(
      ...
      builder: (context) {
        ...
        return AlertDialog(
          title: Text(
            'Detalles de la Baja (${exitType.toUpperCase()})',
            style: AppTextStyles.bodyBold,
          ),
          backgroundColor: AppColors.surface,
          shape: RoundedRectangleBorder(
              ...
              onPressed: () async {
                ...
                if (isSale) {
                  ...
                  if (price <= 0) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text("Ingresa un precio de venta válido"),
                      ),
                    );
                    return;
                  }
                ...
                final row = {
                  ...
                  'reason': observationsController.text.trim().isNotEmpty
                      ...
                      : "Baja por $exitType",
                  'sale_price': price,
                  'buyer': buyerController.text.trim().isNotEmpty
                ...
                Navigator.pop(
                  context,
                ); // Cerrar modal sheet de detalles de animal

                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text(
                      'Baja del animal ${animal.name} registrada exitosamente.',
                      style: AppTextStyles.bodyBold.copyWith(
                        color: Colors.white,
                      ),
                    ),
  ...
  void _showDeregisterConfirmation(
    BuildContext context,
    Animal animal,
    DataProvider provider,
  ) {
      ...
      builder: (sheetContext) {
        return Container(
          ...
          child: Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('Dar de Baja a ${animal.name}', style: AppTextStyles.h2),
              const SizedBox(height: 8),
              Text(
              ...
              ListTile(
                ...
                subtitle: Text(
                  'El animal fue vendido a otra finca o mercado.',
                  style: AppTextStyles.caption,
                ),
                trailing: const Icon(
              ...
              ListTile(
                ...
                subtitle: Text(
                  'El animal falleció debido a enfermedad o accidente.',
                  style: AppTextStyles.caption,
                ),
                trailing: const Icon(
                  LucideIcons.chevronRight,
        ...
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              ElevatedButton.icon(
                onPressed: () {
                  final active = provider.animals
                      .where((a) => a.status.toLowerCase() == 'activo')
                      .toList();
                  if (active.isNotEmpty) {
                    final randomAnimal =
                        active[DateTime.now().second % active.length];
                    tokenController.text =
                        randomAnimal.qrToken ?? randomAnimal.tag;
                  }
                },
              ...
              onPressed: () {
                ...
                if (animal.id.isNotEmpty) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(
                        'Código o Token "$query" no encontrado en el hato local.',
                      ),
...
class _WeightRegisterSheetState extends State<_WeightRegisterSheet> {
  ...
  Widget build(BuildContext context) {
    ...
    if (animalWeights.isNotEmpty) {
      ...
      lastWeightHeader =
          'último peso: ${latestRecord.weightKg.toStringAsFixed(0)} kg hace $daysText';
    } else {
      lastWeightHeader =
          'último peso: ${widget.animal.weightKg.toStringAsFixed(0)} kg (inicial)';
    }
  ...
  Widget _buildRealTimeResultPanel(
    double newWeight,
    WeightRecord? latestRecord,
  ) {
    ...
    if (remaining <= 0) {
      ...
      projectionText =
          'Listo para venta en ~${daysToTarget.round()} días  (aprox. ${_formatDate(targetDate)})';
    }
    ...
    if (compDiff > 0) {
      compText =
          'Por encima del promedio para ${widget.animal.category} (+${compDiff.toStringAsFixed(1)} kg/día)';
    } else if (compDiff < 0) {
      compText =
          'Por debajo del promedio para ${widget.animal.category} (${compDiff.toStringAsFixed(1)} kg/día)';
    } else {
      compText =
          'Igual al promedio para ${widget.animal.category} (+0.0 kg/día)';
    }
            ...
            text: TextSpan(
              ...
              children: [
                ...
                TextSpan(
                  ...
                  style: TextStyle(
                    color: valColor,
                    fontWeight: valColor == AppColors.primaryGreen
                        ? FontWeight.bold
                        : FontWeight.normal,
                  ),
                ),
...
class _DairyProductionSection extends StatefulWidget {
  ...
  @override
  State<_DairyProductionSection> createState() =>
      _DairyProductionSectionState();
}
...
class _DairyProductionSectionState extends State<_DairyProductionSection> {
  ...
  Widget build(BuildContext context) {
    ...
    final todayRecords = animalProds
    ...
    double avgPrev7Days = prevActive7Days > 0
        ? sumPrev7Days / prevActive7Days
        : 11.8;
    ...
    bool showDropAlert =
        todayLiters > 0 && avg7Days > 0 && todayLiters < (avg7Days * 0.85);

    // Preparar datos de 14 días para el gráfico
    List<({DateTime date, double morning, double afternoon, double total})>
    chartData = [];
    ...
    for (int i = 13; i >= 0; i--) {
      ...
      final dayRecords = animalProds
          .where((r) => r.date.startsWith(dateStr))
          .toList();
      double morning = dayRecords
          .where((r) => r.turn == 'Mañana')
          .fold(0.0, (sum, r) => sum + r.liters);
      double afternoon = dayRecords
          .where((r) => r.turn == 'Tarde')
          .fold(0.0, (sum, r) => sum + r.liters);
      double other = dayRecords
          .where((r) => r.turn != 'Mañana' && r.turn != 'Tarde')
          .fold(0.0, (sum, r) => sum + r.liters);
      ...
      children: [
        ...
        Row(
          children: [
            _buildSummaryMetric(
              'Hoy',
              todayLiters > 0
                  ? '${todayLiters.toStringAsFixed(1)} L'
                  : 'Sin registro',
              valueColor: todayLiters > 0
                  ? AppColors.primaryGreen
                  : AppColors.textSecondary,
            ),
            const SizedBox(width: 8),
                                            ...
                                            Container(
                                              ...
                                              decoration: BoxDecoration(
                                                ...
                                                borderRadius: BorderRadius.only(
                                                  ...
                                                  bottomRight:
                                                      const Radius.circular(3),
                                                  topLeft: afternoonHeight == 0
                                                      ? const Radius.circular(3)
                                                      : Radius.zero,
                                                  topRight: afternoonHeight == 0
                                                      ? const Radius.circular(3)
                                                      : Radius.zero,
                                                ),
                                              ),
        ...
        if (animalProds.isEmpty)
          Padding(
            ...
            child: Center(
              child: Text(
                'Sin registros de ordeño previos',
                style: AppTextStyles.caption,
              ),
            ),
          )
        else
          ...List.generate(animalProds.length.clamp(0, 7), (index) {
            ...
            if (index + 1 < animalProds.length) {
              ...
              if (prev.liters > 0) {
                final diff =
                    ((record.liters - prev.liters) / prev.liters) * 100;
                varText =
