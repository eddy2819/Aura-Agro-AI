      ...
      for (var med in medicines) {
        ...
        if (qty <= min) {
          newAlerts.add(AlertModel(
            ...
            description: 'Quedan ${qty.toStringAsFixed(1)} ${med['unit']} (Mínimo: ${min.toStringAsFixed(1)}). Solicita reabastecimiento.',
      ...
      for (var animal in animals) {
        ...
        if (animal.vaccineStatus?.toLowerCase() == 'vencida') {
          newAlerts.add(AlertModel(
            ...
            description: 'El esquema de vacunación de ${animal.name} está vencido. Programa vacunación.',
        ...
        final animalWeights = weightRecords.where((w) => w['animal_id'] == animal.id).toList();
        if (animalWeights.length >= 2) {
          animalWeights.sort((a, b) => (b['date'] as String).compareTo(a['date'] as String));
          final double lastWeight = (animalWeights[0]['weight_kg'] as num).toDouble();
          final double avgRecent = animalWeights.skip(1).map((w) => (w['weight_kg'] as num).toDouble()).reduce((a, b) => a + b) / (animalWeights.length - 1);
          if (lastWeight < avgRecent * 0.9) {
            newAlerts.add(AlertModel(
              ...
              description: 'El peso bajó a ${lastWeight.toStringAsFixed(1)} kg (Promedio reciente: ${avgRecent.toStringAsFixed(1)} kg). Revisa nutrición o descarta parasitosis.',
        ...
        final animalProds = productionRecords.where((p) => p['animal_id'] == animal.id).toList();
        if (animalProds.length >= 4) {
          animalProds.sort((a, b) => (b['date'] as String).compareTo(a['date'] as String));
          ...
          final double avgPrev = animalProds.skip(3).take(5).map((p) => (p['liters'] as num).toDouble()).reduce((a, b) => a + b) / min(5, animalProds.length - 3);
          ...
          if (l1 < avgPrev * 0.9 && l2 < avgPrev * 0.9 && l3 < avgPrev * 0.9) {
            newAlerts.add(AlertModel(
              ...
              description: 'Producción láctea en descenso sostenido por 3 días (${l1.toStringAsFixed(1)} L vs promedio histórico de ${avgPrev.toStringAsFixed(1)} L). Posible indicio de mastitis.',
        ...
        final animalRepros = reproRecords.where((r) => r['animal_id'] == animal.id).toList();
        for (var rep in animalRepros) {
          if (rep['event_type'] == 'Gestación confirmada' && rep['expected_birth_date'] != null) {
            try {
              final birthDate = DateTime.parse(rep['expected_birth_date'] as String);
              ...
              if (hoursDiff > 0 && hoursDiff <= 48) {
                newAlerts.add(AlertModel(
                  ...
                  description: 'Fecha esperada de parto dentro de ${hoursDiff} horas (${rep['expected_birth_date']}). Prepara cubículo de parición.',
        ...
        final animalSos = sosCases.where((s) => s['animal_id'] == animal.id).toList();
        if (animalSos.isNotEmpty) {
          animalSos.sort((a, b) => (b['created_at'] as String).compareTo(a['created_at'] as String));
          ...
          if (risk == 'rojo') {
            newAlerts.add(AlertModel(
              ...
              description: 'Síntoma: ${latestSos['symptoms_text']}. Requiere atención veterinaria urgente.',
              source: 'SOS Engine',
